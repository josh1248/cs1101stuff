...
function dial_all(list_of_numbers) {
    ...
    //Filter out all evil numbers
    const non_evil_numbers = 
        filter(numlist => digits_to_num(numlist) !== evil_num,
               list_of_numbers);
               
    //Convert list of numbers to their representative sound
    const number_sounds = map(dial, non_evil_numbers);
    
    //Top off all number sounds with the end_sound of #
    const number_sounds_with_end = 
            map(number_sound => consecutively(list(number_sound,end_sound)),
                number_sounds);
    
    //combine processed number sounds together
    return consecutively(number_sounds_with_end);
}
