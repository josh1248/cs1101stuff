...
//the two helper functions in the original share such a similar structure that
//i applied a generalisation function to it!
function generate_value(input_element, modified_list, getter) {
    return is_null(modified_list)
           ? 0
           : input_element <= head_entry_of(modified_list)
           ? generate_value(input_element, tail(modified_list), getter)
           : getter(modified_list) + 
             generate_value(input_element, tail(modified_list), getter);
}

//Input: input element, modified list.
//Output: tag that accompanies input element.
function generate_tag_of(input_element, modified_list) {
    return generate_value(input_element, modified_list, entry => 1);
}
...
function triplets_with_element(input_element, modified_list) {
    return generate_value(input_element, modified_list, entry => head_tag_of(entry));
}
